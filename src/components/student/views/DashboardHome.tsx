    ...
    return (
        <div className="relative grid grid-cols-1 lg:grid-cols-12 gap-8 pb-20">
            ...
            <div className="lg:col-span-8 space-y-8">
                ...
                {lastAccessedInfo && (
                    <Card className="p-7 bg-cyan-500/5 border-cyan-500/30 rounded-[2.5rem] flex flex-col md:flex-row items-center gap-8 animate-fade-in group hover:bg-cyan-500/10 transition-all duration-500 shadow-xl shadow-cyan-500/5">
                        <div className="h-20 w-20 rounded-3xl bg-cyan-500 flex items-center justify-center shadow-[0_0_30px_rgba(6,182,212,0.4)] flex-shrink-0 group-hover:scale-110 transition-transform">
                            <CycleIcon className="h-10 w-10 text-white" />
                        </div>
                        <div className="flex-grow text-center md:text-left min-w-0">
                            <span className="text-[10px] font-black text-cyan-500 uppercase tracking-[0.4em] mb-1.5 block">Retomar Jornada</span>
                            <h4 className="text-2xl font-black text-white uppercase tracking-tighter leading-none truncate italic">
                                {lastAccessedInfo.subtopic?.name || lastAccessedInfo.topic.name}
                            </h4>
                            <p className="text-xs text-gray-500 font-bold mt-2 uppercase tracking-widest opacity-60">
                                {lastAccessedInfo.subject.name}
                            </p>
                        </div>
                        <Button 
                            onClick={() => onNavigateToTopic(studentProgress.lastAccessedTopicId!)}
                            className="w-full md:w-auto py-5 px-12 text-xs font-black uppercase tracking-[0.3em] shadow-2xl rounded-2xl bg-gradient-to-r from-cyan-600 to-blue-700"
                        >
                            ESTUDAR AGORA
                        </Button>
                    </Card>
                ...
                <Card className="p-8 bg-gray-800/40 border-gray-700/50 rounded-[2.5rem]">
                    <div className="flex items-center gap-3 mb-8">
                        <div className="w-1.5 h-8 bg-cyan-500 rounded-full shadow-[0_0_12px_cyan]"></div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        {enrolledCourses.map(course => {
                            ...
                            return (
                                <Card key={course.id} onClick={() => onCourseSelect(course)} className="group hover:border-cyan-500/50 transition-all duration-500 flex flex-col !p-0 overflow-hidden bg-gray-900/60 shadow-2xl rounded-[2.2rem] hover:translate-y-[-4px]">
                                    {course.imageUrl ? (
                                    ...
                                    <div className="p-8 flex-grow flex flex-col">
                                        <h4 className="text-2xl font-black text-white group-hover:text-cyan-400 transition-colors flex-grow leading-tight uppercase tracking-tight italic">{course.name}</h4>
                                        <div className="mt-8 flex items-center justify-between border-t border-gray-800/50 pt-6">
                                            <div className="flex items-center gap-3">
                                                {teacher?.avatarUrl ? (
                                                    <img src={teacher.avatarUrl} alt={teacher.name} className="h-8 w-8 rounded-full object-cover ring-2 ring-gray-700 shadow-md" />
                                                ) : (
                                                    <div className="w-8 h-8 rounded-full bg-gray-700 flex items-center justify-center border border-gray-600">
                                                        <UserCircleIcon className="h-5 w-5 text-gray-400" />
                                                    </div>
                                                )}
                                                <span className="text-[10px] font-black uppercase text-gray-400 tracking-widest group-hover:text-cyan-400 transition-colors">
                                                    Prof. {teacher?.name?.split(' ')[0] || 'QG'}
                                                </span>
                                            </div>
                                            <div className="text-cyan-400 text-[9px] font-black uppercase tracking-[0.2em] flex items-center gap-2 group-hover:translate-x-1 transition-all">
                ...
                <div className="space-y-6">
                    ...
                    <div className="space-y-4 max-h-[550px] overflow-y-auto pr-2 custom-scrollbar">
                        {broadcasts.length > 0 ? broadcasts.map(msg => {
                            ...
                            return (
                                <div key={msg.id} className={`group relative p-6 rounded-[2rem] border transition-all duration-300 ${isUnread ? 'bg-orange-500/5 border-orange-500/20 shadow-lg shadow-orange-500/5' : 'bg-gray-800/40 border-gray-700/40'}`}>
                                    <button onClick={() => onDeleteMessage(msg.id)} className="absolute top-5 right-5 p-2 text-gray-600 hover:text-red-400 opacity-0 group-hover:opacity-100"><TrashIcon className="h-5 w-5" /></button>
                                    <div className="flex gap-4 items-start" onClick={() => isUnread && onAcknowledgeMessage(msg.id)}>
                                        <div className="flex-shrink-0">
                                            {teacher?.avatarUrl ? <img src={teacher.avatarUrl} alt="" className="h-10 w-10 rounded-2xl object-cover" /> : <div className="h-10 w-10 rounded-2xl bg-gray-700 flex items-center justify-center"><UserCircleIcon className="h-6 w-6 text-gray-500" /></div>}
                                        </div>
                                        <div className="flex-grow min-w-0">
                                            <p className="text-[10px] font-black text-cyan-400 uppercase tracking-widest truncate">Prof. {teacher?.name || 'QG'}</p>
                                            <p className={`text-sm mt-1.5 leading-relaxed ${isUnread ? 'text-white font-bold' : 'text-gray-400'}`}>{msg.message}</p>
                                        </div>
                                    </div>
                                </div>
                            );
                        }) : <p className="text-center text-gray-600 text-[10px] font-black uppercase py-12 border-2 border-dashed border-gray-800 rounded-[2.5rem] tracking-[0.2em]">Nenhum aviso ativo</p>}
                    </div>
                </div>
            </div>
            ...
            <button
                ...
                className="fixed bottom-10 right-10 w-16 h-16 bg-gradient-to-br from-cyan-500 to-blue-600 rounded-[1.8rem] shadow-[0_15px_40px_-10px_rgba(6,182,212,0.6)] flex items-center justify-center text-white hover:scale-110 active:scale-90 transition-all z-50 border border-white/10"
            >
