    ...
    const navigationItems = useMemo(() => {
        if (isProfessorView) {
            return [
                ...
                { label: 'Revisões da Turma', view: 'reviews' },
                { label: 'Planejamento Mensal', view: 'scheduler' },
                { label: 'Desempenho Geral', view: 'performance' },
    ...
    return (
        <header className="sticky top-0 z-50 w-full bg-[#020617] border-b border-white/5 shadow-[0_15px_50px_-15px_rgba(0,0,0,0.8)] h-24">
            <div className="w-full h-full px-6 sm:px-12 md:px-16 lg:px-24 flex items-center justify-between gap-4">
                ...
                {/* 3. ELEMENTO CENTRAL (PERFORMANCE OU IDENTIFICADOR) */}
                {!isProfessorView ? (
                        ...
                        <button onClick={() => onSetView('performance')} className="flex items-center gap-3 hover:bg-white/5 p-2 rounded-2xl transition-all group">
                            <div className="relative h-12 w-12">
                                ...
                                <div className="absolute inset-0 flex flex-col items-center justify-center leading-none">
                                    <span className="text-[14px] font-black text-white">{level}</span>
                                    <span className="text-[6px] font-black text-cyan-500 uppercase">LVL</span>
                                </div>
                            </div>
                            <div className="flex flex-col text-left leading-none">
                                <span className="text-[13px] font-black text-white uppercase italic tracking-tighter group-hover:text-cyan-400 transition-colors">{levelTitle}</span>
                ...
                <div className="flex-shrink-0 hidden lg:flex items-center h-12 bg-black/50 rounded-full border border-white/10 px-8 gap-8 shadow-inner">
                    ...
                    <div className="flex flex-col items-center">
                        <span className="text-[7px] font-black text-cyan-500 uppercase tracking-[0.2em] mb-0.5">{isProfessorView ? 'ONLINE' : 'SESSÃO'}</span>
                ...
                {/* 5. PERFIL (ESTILO EXATO DA REFERÊNCIA VISUAL) */}
                <div ref={navRef} className="flex-shrink-0 relative flex items-center">
                    <button 
                        onClick={() => setIsNavOpen(prev => !prev)} 
                        className={`flex items-center gap-3 p-1.5 pr-5 lg:pr-8 rounded-full bg-gray-900/80 border border-white/10 hover:border-cyan-500/40 transition-all duration-300 ${isNavOpen ? 'ring-2 ring-cyan-500/30 bg-gray-800' : ''}`}
                    >
                        <div className="relative flex-shrink-0">
                            <div className="h-10 w-10 lg:h-12 lg:w-12 rounded-full overflow-hidden border-2 border-cyan-500/60 shadow-[0_0_15px_rgba(6,182,212,0.3)] bg-gray-800">
                                {user.avatarUrl ? (
