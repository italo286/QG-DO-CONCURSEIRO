...
export const StudentHeader: React.FC<StudentHeaderProps> = ({
    ...
    return (
        <header className="sticky top-0 z-50 w-full bg-[#020617] border-b border-white/5 shadow-[0_10px_30px_-15px_rgba(0,0,0,0.5)] h-20">
            <div className="max-w-[1920px] mx-auto h-full px-4 flex items-center justify-center gap-2 md:gap-4 lg:gap-6">
                ...
                {/* 2. NOME DO SITE */}
                <span className="text-lg md:text-xl lg:text-2xl xl:text-3xl font-black text-white uppercase tracking-tighter leading-none whitespace-nowrap drop-shadow-[0_0_15px_rgba(255,255,255,0.1)]">
                    QG do concurseiro
                </span>

                {/* 3. HUD DE NÍVEL (VISÍVEL NO TABLET/PC) */}
                <button onClick={() => onSetView('performance')} className="hidden md:flex items-center gap-1.5 hover:bg-white/5 p-1 rounded-xl group transition-all flex-shrink-0">
                ...
                {/* 4. HUD DE ACERTO (VISÍVEL NO TABLET/PC) */}
                <button onClick={() => onSetView('performance')} className="hidden md:flex items-center gap-1.5 hover:bg-white/5 p-1 rounded-xl group transition-all flex-shrink-0">
                    <div className={`relative h-9 w-9 lg:h-11 lg:w-11 ${accTheme.shadow} rounded-full`}>
                        <svg className="h-full w-full -rotate-90" viewBox="0 0 36 36">
                ...
                {/* 5. RELÓGIO (APENAS PC PARA EVITAR APERTO NO TABLET) */}
                <div className="hidden lg:flex items-center h-9 bg-black/40 rounded-full border border-white/5 px-3 gap-3 flex-shrink-0">
